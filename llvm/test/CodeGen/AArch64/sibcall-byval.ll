...
; Ported from X86's sibcall-byval.ll. A plain (C calling convention) tail call
; that forwards an incoming byval argument unchanged to the callee. The byval
; stays at the same stack offset, so the sibling call is performed (jump, not
; call) even though the caller has a byval argument.
